...
<head>
	...
	<title>メッセージボード・ユーザー評価管理画面</title>
	
	<script src="{{ asset('/js/app.js') }}" defer></script>
	<script src="{{ $crud_base_js . $ver_str }}" defer></script>
	<script src="{{ asset('/js/MsgBoardUserEval/index.js')  . $ver_str }}" defer></script>
	...
	<link href="{{ asset('/css/MsgBoardUserEval/index.css') . $ver_str }}" rel="stylesheet">
...
<nav aria-label="breadcrumb">
  <ol class="breadcrumb">
	...
	<li class="breadcrumb-item active" aria-current="page">メッセージボード・ユーザー評価管理画面</li>
  </ol>
</nav>
...
<div class="cb_func_line">
	...
	<!-- 検索条件入力フォーム -->
	<div class="form_kjs" id="msg_board_user_evalIndexForm" method="post" accept-charset="utf-8">
		
		<?php $cbh->inputKjMain('kj_main','',null,'メッセージボード・ユーザー評価名、備考を検索する');?>
		...
		$cbh->inputKjSelect('kj_eval_type_id','評価種別ID', $masters['evalTypeIdList']); 
	...
	<div id="main_tools" style="margin-bottom:10px;margin-top:4px">
		<div style="display:inline-block;width:75%; ">
			<?php 
				...
				// CSVエクスポート機能
	 			$csv_dl_url =  'msg_board_user_eval/csv_download';
...
<!-- 一覧テーブル -->
<table id="msg_board_user_eval_tbl" class="table table-striped table-bordered table-condensed" style="display:none;margin-bottom:0px">
...
foreach($data as $i=>&$ent){
	...
	$cbh->tdList($ent, 'eval_type_id', $evalTypeIdList);
...
<table id="crud_base_forms">
	...
	<tr id="ajax_crud_new_inp_form" class="crud_base_form" style="display:none;padding-bottom:60px"><td colspan='5'>
		...
		<div class="cbf_inp_wrap">
			<div class='cbf_inp_label' >評価種別ID: </div>
			<div class='cbf_input'>
				<?php $cbh->selectX('eval_type_id',null,$evalTypeIdList,null);?>
				<label class="text-danger" for="eval_type_id"></label>
			</div>
		</div>
...
<div id="ajax_crud_delete_form" class="panel panel-danger" style="display:none">
	...
	<table><tbody>
		...
		<tr><td>メッセージボード・ユーザー評価名: </td><td>
			<span class="msg_board_user_eval_name"></span>
		</td></tr>
